...
describe("CodexDeployer.deployRepository skills", () => {
  ...
  it("copies without confirmation when target does not exist", async () => {
    ...
    const deployer = new CodexDeployer();

    await deployer.deployRepository(makeRepository([skill]), "/target", false);

    expect(confirmSpy).not.toHaveBeenCalled();
    expect(copyToSpy).toHaveBeenCalledWith("/target/.codex/skills/my-skill/");
  });
...
describe("CodexDeployer.deployRepository agents", () => {
  let copyToSpy: ReturnType<typeof spyOn>;
